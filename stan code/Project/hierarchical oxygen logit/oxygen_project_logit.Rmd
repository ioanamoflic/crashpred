...
fit_result <- stan(file = "hierarchical_model_oxygen_logit.stan", data = stan_data)
...
```{r}
posterior <- as.array(fit_result)
mcmc_trace(posterior, pars = c('intercept[1]', 'intercept[2]', 'year_weight[1]', 'year_weight[2]'))

```

```{r}
posterior <- as.array(fit_result)
mcmc_dens(posterior, pars = c('intercept[1]', 'intercept[2]', 'year_weight[1]', 'year_weight[2]'))
```

```{r}
#ess_basic(extract(fit_result))


ess_basic(extract(fit_result)$year_weight[, 1])
ess_basic(extract(fit_result)$year_weight[, 2])

ess_basic(extract(fit_result)$intercept[, 1])
ess_basic(extract(fit_result)$intercept[, 2])

...
transformed parameters {
  ...
  for(i in 1:N)
    for(j in 1:M)
    {
      ...
      theta[i, j] = ((intercept[j] + year_weight * (year[i, j] - 1980)/8 + oxygen_weight * oxygen_used[i, j])/100 - 0.5)*12;
    }
...
model {
  // hyper prior
  hyper_intercept_mean ~ normal(40, 10); //uniform(-4, 4); //uniform(40, 80); //normal(70, 5);
  ...
  year_weight ~ normal(4, 1); //uniform(0, 4); //normal(0.4, 0) //normal(4, 0.75); //uniform(0, 5); //normal(0,20);

...
transformed parameters {
  ...
  for(i in 1:N)
    for(j in 1:M)
    {
      theta[i, j] = 1/(1+exp( - (intercept[j] + year_weight[j] * (year[i, j] - 1980)/8)/100 ));
    }
...
model {
  // hyper prior
  hyper_intercept_mean ~ normal(0, 100); //normal(0, 500); //normal(0, 100); //normal(0, 100); 
  hyper_intercept_sd ~ normal(100, 30); //normal(500, 100); //normal(100, 30); //normal(0, 50); 
  
  hyper_year_weight_mean ~ normal(0, 20); //normal(0, 100); //normal(0, 20); //normal(0, 10); 
  hyper_year_weight_sd ~ normal(30, 10); //normal(150, 50); //normal(30, 10); //normal(10, 3);
